...
func (k *kataAgent) createContainer(sandbox *Sandbox, c *Container) (p *Process, err error) {
	...
	//vmiStorages := k.handleVmiStorage(ociSpec.Mounts, sandbox.id, c.rootfsSuffix,rootPath)
	//ctrStorages = append(ctrStorages, vmiStorages...)
...
func (k *kataAgent) handleVmiStorage(mounts []specs.Mount, sandboxID string, rootfsSuffix string, rootPath string) []*grpc.Storage {
	var vmiStorages []*grpc.Storage
        // Set the mount source path to a the desired directory point in the VM.
	//mountPoint := filepath.Join(kataGuestSharedDir(), sandboxID, rootfsSuffix, "/var/sockets")
	mountPoint := filepath.Join(rootPath, "/var/sockets")
	k.Logger().Error("vmi mountPoint %s", mountPoint)

	// Create a storage struct so that the kata agent is able to create the
	// directory inside the VM.
	vmiStorage := &grpc.Storage{
				Driver:     KataEphemeralDevType,
				Source:     "/run/sockets",
				Fstype:     "bind",
				MountPoint: mountPoint,
			}
	vmiStorages = append(vmiStorages, vmiStorage)
	return vmiStorages
}
...
func (k *kataAgent) handleDeviceBlockVolume(c *Container, device api.Device) (*grpc.Storage, error) {
	...
	//k.Logger().Error("vol.Source is %s",vol.Source)
	return vol, nil
}
...
func (k *kataAgent) handleBlockVolumes(c *Container) ([]*grpc.Storage, error) {
	...
	for _, m := range c.mounts {
		...
		if vol == nil || err != nil {
	        ...
	        //k.Logger().WithField("device", id).Error("m.Destination or mountPoint is %s",m.Destination)

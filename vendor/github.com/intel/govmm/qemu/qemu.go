...
func (netdev NetDevice) QemuNetdevParams(config *Config) []string {
	...
	log.Printf("netdev.Type %s ", netdev.Type)
	if netdevType == "" {
		return nil
	}
...
func (netdev NetDevice) QemuParams(config *Config) []string {
	...
	//log.Fatal("netdev.Type %s ", netdev.Type)
	//if (netdev.Type == MACVTAP) {
	//     log.Fatal("netdev.Type %s ", MACVTAP)
	//}
...
func (config *Config) appendInput() {
	//config.qemuParams = append(config.qemuParams, "-device usb-ehci -device usb-tablet  -device usb-kbd -device usb-mouse -usb")
	config.qemuParams = append(config.qemuParams, "-device")
	config.qemuParams = append(config.qemuParams, "usb-ehci")
	config.qemuParams = append(config.qemuParams, "-device")
	config.qemuParams = append(config.qemuParams, "usb-tablet")
	config.qemuParams = append(config.qemuParams, "-device")
	config.qemuParams = append(config.qemuParams, "usb-kbd")
	config.qemuParams = append(config.qemuParams, "-device")
	config.qemuParams = append(config.qemuParams, "usb-mouse")
	config.qemuParams = append(config.qemuParams, "-usb")
}
